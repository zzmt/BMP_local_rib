...
import json
import configparser
from tool import falcon_alert,local_sysname_port
import requests
...
MSG_PROCESS_OPTS = [
    cfg.StrOpt(
        'write_dir',
        default=os.path.join(os.environ.get('HOME', './'), 'BMP/msg'),
...
base_url = 'http://beidou.sankuai.com/robot/v1/bmp/'
dci_url = base_url + 'network_link'
...
class DefaultHandler(BaseHandler):
    ...
    def on_connection_made(self, peer_host, peer_port):
        ...
        #    log('connection made: peer %s',peer_host)
    ...
    def on_message_received(self, peer_host, peer_port, msg, msg_type):
        ...
        if msg_type == 0:  # route monitoring message
            ...
            print(msg[1][1])
            nlri_list = msg[1][1].get('nlri',[])
            withdraw_list = msg[1][1].get('withdraw',[])

            if nlri_list or withdraw_list:
                attr = msg[1][1].get('attr')
                if nlri_list:
                    for nlri in nlri_list:
                        bgp_info = {peer_host+'_'+nlri.get('prefix','none')+'_'+str(nlri.get('path_id','none')):{'attr':attr}}
                        print('hset'+'-'+json.dumps(bgp_info))
                if withdraw_list:
                    for withdraw in withdraw_list:
                        bgp_info = {peer_host+'_'+withdraw.get('prefix','none')+'_'+str(withdraw.get('path_id','none')): {'attr':attr}}
                        print('del'+'-'+json.dumps(bgp_info))
            ...
            up_temp = {}
            up_temp['status']='up'
            up_temp['peer_ip']=peer_ip
            up_msg[peer_host]= up_temp
